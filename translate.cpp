...
char command[] = "pocketsphinx_continuous -inmic yes -adcdev plughw:1,0 -samprate 16000 -logfn /dev/null -hmm (hmm path) -lm (lm path) -dict (dict path) > (text file)";

// enter api-key 
char sendcurl[] = "curl --user apikey:(your key) --request POST --header \"Content-Type: application/json\" --data \"{\\\"text\\\"\:";

// url must be adjusted
char endtext[] = "\\\"\model_id\\\"\:\\\"es-en\\\"\}\"\ \"\(your URL)\"";
...
int main()
{
	...
	sleep(5);                                                                 	// sleep for 5 seconds on Linux/Unix systems for "foo.txt" to be created
        system("pacmd set-card-profile bluez_card.(Bluetooth address) a2dp_sink");	// bluetooth address must be adjusted
        system("pacmd set-default-sink bluez_sink.(Bluetooth address).a2dp_sink");      // bluetooth address must be adjusted
	
        std::ifstream myfile("(text file)");                       		        // reads file pocketsphinx outputs to
	std::string templine;
	...
	while(1)
	{
		...
		while(getline(myfile, templine))
		{
			...
			while (!(exists("(wav file)")))	         		       // if the wav file is not created yet, sleep until it is created
                        {
                                sleep(1);
                        }
                        
			system("omxplayer -o alsa (wav file)"); 		       // sound player must be changed depending on system (omxplayer for linux)	
